...
from preprocessing.refactorings import (
    ExtractInlineRef,
    RenameRef,
    MoveRef,
    ClassRef,
...
def extract_method_ref(common_methods, added_methods):
    ...
    for tuple_m in common_methods:
        for added_m in added_methods[:]:
            ...
            method2 = tuple_m[1]
            if (
                not method1.calls(added_m)
                and method1.class_node == added_m.class_node
                and method2.calls(added_m)
            ):
                ...
                method1_stmts = [
                    {"stmt1": str(stmt)} for stmt in method1.get_all_stmts()
                ]
                method2_stmts = [
                    {"stmt1": str(stmt)} for stmt in method2.get_all_stmts()
                ]

                method1_stmts_df = (
                    pd.DataFrame(method1_stmts)
                    .groupby("stmt1")["stmt1"]
                    .agg("count")
                    .reset_index(name="count")
                )
                method2_stmts_df = (
                    pd.DataFrame(method2_stmts)
                    .groupby("stmt1")["stmt1"]
                    .agg("count")
                    .reset_index(name="count")
                )

                stmts_merge = method1_stmts_df.merge(
                    method2_stmts_df, how="outer", on="stmt1"
                ).fillna(0)

                mapped_stmts_agg = (
                    mapped_stmts.groupby("stmt1")["stmt1"]
                    .agg("count")
                    .reset_index(name="count")
                )

                mapped_stmts_merge = mapped_stmts_agg.merge(
                    stmts_merge, how="outer", on="stmt1"
                ).dropna()

                to_remove = mapped_stmts_merge[
                    ~(
                        abs(mapped_stmts_merge["count"] - mapped_stmts_merge["count_x"])
                        >= mapped_stmts_merge["count_y"]
                    )
                ]["stmt1"].tolist()

                mapped_stmts = mapped_stmts[~(mapped_stmts["stmt1"].isin(to_remove))]

                mapped_stmts_len = len(
                    mapped_stmts[~mapped_stmts.type.str.contains("inner")].index
                )
...
def change_class_signature(
    removed_classes: list, added_classes: list, common_classes: list
):
    refs = []
    sub_refs = []
    for removed_class in removed_classes[:]:
        for added_class in added_classes[:]:
            ...
            removed_class_methods = set(
                [method.name for method in removed_class.methods]
            )
            ...
            common_methods_len = len(
                removed_class_methods.intersection(added_class_methods)
            )
            total_methods_len = (
                len(removed_class_methods)
                if len(removed_class_methods) > len(added_class_methods)
                else len(added_class_methods)
            )

            if (
                (
                    removed_class_methods.issubset(added_class_methods)
                    or added_class_methods.issubset(removed_class_methods)
                )
                and (
                    removed_class_fields.issubset(added_class_fields)
                    or added_class_fields.issubset(removed_class_fields)
                )
            ) or (
                common_methods_len > total_methods_len / 2
                and (
                    removed_class_fields.issubset(added_class_fields)
                    or added_class_fields.issubset(removed_class_fields)
                )
                ...
                if not (removed_class.name == added_class.name):
                    ...
                    sub_refs.append(
                        {
                            "Removed Class": removed_class.name,
                            "Added Class": added_class.name,
                            "Matched Method": matched_method_ratio,
                            "Name Similarity": editdistance.eval(
                                added_class.name, removed_class.name
                            ),
                            "Ref": ClassRef(
                                removed_class.name,
                                added_class.name,
    ...
    if len(sub_refs.index) > 0:
        sub_refs = (
            sub_refs.sort_values(
                ["Matched Method", "Name Similarity"], ascending=[False, True]
            )
            .groupby("Added Class")
            .head(1)
        )
...
def inline_method_ref(common_methods, removed_methods):
    ...
    for tuple_m in common_methods:
        for removed_m in removed_methods[:]:
            ...
            if (
                method1.calls(removed_m)
                and adjacent_method.class_node == removed_m.class_node
                and not adjacent_method.calls(removed_m)
            ):
                ...
                method2_stmts = [
                    {"stmt1": str(stmt)} for stmt in adjacent_method.get_all_stmts()
                ]
                ...
                method1_unmapped = (
                    adjacent_method.get_total_stmts_count() - mapped_stmts_len
                )
...
def move_method_ref(diff_common_element):
    ...
    for removed_method in removed_methods:
        ...
        for added_method in added_methods:
            ...
            method1_unmapped = abs(
                removed_method.get_total_stmts_count() - mapped_stmts_len
            )
            method2_unmapped = abs(
                added_method.get_total_stmts_count() - mapped_stmts_len
            )
            if (
                mapped_stmts_len >= method1_unmapped
                and mapped_stmts_len >= method2_unmapped
                and added_method.name == removed_method.name
            ):
                priority = len(mapped_stmts[mapped_stmts["replacements"].isnull()])
                total_distance = mapped_stmts["distance"].sum()
                if len(metrics) == 0:
                    metrics = [
                        priority / mapped_stmts_len,
                        total_distance,
                        mapped_stmts_len,
                    ]
                else:
                    if not (
                        priority / mapped_stmts_len >= metrics[0]
                        and total_distance <= metrics[1]
                        and mapped_stmts_len >= metrics[2]
                    ):
                        continue
                    else:
                        sub_refs = []
                ...
                if not (removed_method.module.name == added_method.module.name):
                    if (
                        removed_method.class_node is not None
                        and added_method.class_node is not None
                    ):
                        if (
                            added_method.class_node.name
                            in removed_method.class_node.bases
                        ):
                            _move = "Pull Up"
                        if (
                            removed_method.class_node.name
                            in added_method.class_node.bases
                        ):
                            _move = "Push Down"
                    sub_refs.append(
                        MoveRef(
                            ...
                            added_method.name,
                            "Move Method",
                            added_method.get_path(),
                            removed_method.get_path(),
                            _move,
                            mapped_stmts,
                            removed_method,
                            added_method,
                    ...
                    removed_method.class_node is not None
                    or added_method.class_node is not None
                ):
                    sub_refs.append(
                        MoveRef(
                            removed_method.name,
...
def method_signature_change_ref(added_methods, removed_methods, common_methods):
    ...
    for removed_method in removed_methods:
        for added_m in added_methods:
            ...
            other_added_methods = [
                added_m for added_m in added_methods if not added_m.name == added_m.name
            ]
            extracted_refs = extract_method_ref(
                [(removed_method, added_m)], other_added_methods
            )
            refs.extend(extracted_refs)
            if added_m.class_node == removed_method.class_node and (
                (method1_unmapped == 0 and method2_unmapped == 0)
                or (
                    (
                        mapped_stmts_len >= method1_unmapped
                        and mapped_stmts_len >= method2_unmapped
                    ...
                    and (
                        set(added_m.params).issuperset(set(removed_method.params))
                        or set(removed_method.params).issuperset(set(added_m.params))
                        or len(
                            intersection(
                                set(added_m.params), set(removed_method.params)
                            )
                        )
                        > len(
                            set(added_m.params + removed_method.params).difference(
                                intersection(
                                    set(added_m.params), set(removed_method.params)
                                )
                            )
                ...
                if (
                    len(set(added_m.params).difference(set(removed_method.params))) > 0
                    or len(set(removed_method.params).difference(set(added_m.params)))
                    > 0
                ):
                    pre_params = [
                        i for i in removed_method.params if i not in added_m.params
                    ]
                    post_params = [
                        i for i in added_m.params if i not in removed_method.params
                    ]
                ...
                if len(_changes) > 0:
                    matched_methods = matched_methods._append(
                        {
                            "from": removed_method.name,
                            "to": added_m.name,
                            "ref_type": _changes,
                            "priority": priority,
                            "total_distance": total_distance,
                            "path": added_m.get_path(),
                            "m1": removed_method,
                            "m2": added_m,
                            "mapped_stmts": mapped_stmts,
    ...
    if len(matched_methods.index) > 0:
        ...
        for name, group in gp:
            toKeep.append(
                group.sort_values(
                    ["priority", "total_distance"], ascending=[False, True]
                ).iloc[0]
            )
    ...
    for matched_method in matched_methods:
        ...
        added_m_rm = [
            added_m for added_m in added_methods if added_m.name == matched_method[1]
        ]
        remove_m_rm = [
            removed_m
            for removed_m in removed_methods
            if removed_m.name == matched_method[0]
        ]
